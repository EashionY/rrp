	   ...
	   $.get(ip+"/rrp/resume/findByUserId.do",{userId:userId},function(data){
		   if(data.state==0){
			 ...
			 //  console.log(resumeId);
			   $.get(ip+"/rrp/delivery/viewDeliveried.do",{resumeId:resumeId,deliveryStatus:4},function(result){
				   if(result.state==0){
					   if(result.data.length!=0){
						   ...
						   $.each(result.data,function(k,v){
							   ...
							   str+='<div class="delivery_leftcontbox"><div class="delivery_leftcontmain">'+
			                    ...
			                    '<span>不合适 </span><span class="delivery_div2_time">'+new Date(v.unfitTime).format("yyyy-MM-dd hh:mm")+'</span>'+
			                    ...
			                    '<div class="delivery_state">不合适</div><div class="delivery_time">'+new Date(v.unfitTime).format("yyyy-MM-dd hh:mm")+'</div>'+
