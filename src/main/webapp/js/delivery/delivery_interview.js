	   ...
	   $.get(ip+"/rrp/resume/findByUserId.do",{userId:userId},function(data){
		   if(data.state==0){
			 ...
			 //  console.log(resumeId);
			   $.get(ip+"/rrp/delivery/viewDeliveried.do",{resumeId:resumeId,deliveryStatus:3},function(result){
				   if(result.state==0){
					   if(result.data.length!=0){
						   ...
						   $.each(result.data,function(k,v){
							   str+='<div class="delivery_leftcontbox"><div class="delivery_leftcontmain">'+
			                    ...
			                    '<span class="delivery_div2_time">'+new Date(v.inviteTime).format("yyyy-MM-dd hh:mm")+'</span><span class="myfont delivery_div2_font">&#xe681;</span></div></div>'+
			                    ...
			                    '<div class="delivery_time">'+new Date(v.deliveyTime).format("yyyy-MM-dd hh:mm")+'</div><div class="delivery_time"><span class="delivery_tip"></span>HR已收到你的简历</div></div>'+
			                    ...
			                    '<div class="delivery_time">'+new Date(v.checkTime).format("yyyy-MM-dd hh:mm")+'</div>'+
			                    ...
			                    '<div class="delivery_state">对我有意</div><div class="delivery_time">'+new Date(v.intendTime).format("yyyy-MM-dd hh:mm")+'</div><div class="delivery_time">'+
			                    '<span class="delivery_tip"></span>你的简历通过初选</div></div><div class="delivery_leftdet_div delivery_leftdet_divactiv">'+
			                    '<div class="delivery_number">4</div><div class="delivery_state">邀请面试</div>'+
			                    '<div class="delivery_time">'+new Date(v.inviteTime).format("yyyy-MM-dd hh:mm")+'</div>'+
