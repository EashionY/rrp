	   ...
	   $.get(ip+"/rrp/resume/findByUserId.do",{userId:userId},function(data){
		   if(data.state==0){
			 ...
			 //  console.log(resumeId);
			   $.get(ip+"/rrp/delivery/viewDeliveried.do",{resumeId:resumeId,deliveryStatus:2},function(result){
				   if(result.state==0){
					   if(result.data.length!=0){
						   ...
						   $.each(result.data,function(k,v){
							   ...
							   str+='<div class="delivery_leftcontbox"><div class="delivery_leftcontmain"><div class="delivery_leftcontdiv1">'+
			                        ...
			                        '<span>对我有意 </span><span class="delivery_div2_time">'+new Date(v.intendTime).format("yyyy-MM-dd hh:mm")+'</span><span class="myfont delivery_div2_font">&#xe681;</span></div></div>'+
			                        '<div class="delivery_leftcontdetail"><div class="delivery_leftdet_div"> <div class="delivery_number delivery_numberactive">1</div>'+
			                        '<div class="delivery_state">投递成功</div><div class="delivery_time">'+new Date(v.deliveyTime).format("yyyy-MM-dd hh:mm")+'</div>'+
			                        ...
			                        '<div class="delivery_state">已被查看</div><div class="delivery_time">'+new Date(v.checkTime).format("yyyy-MM-dd hh:mm")+'</div>'+
			                        ...
			                        '<div class="delivery_state">对我有意</div><div class="delivery_time">'+new Date(v.intendTime).format("yyyy-MM-dd hh:mm")+'</div>'+
